...
async def httpx_req(list_url,req_1_w,):
    async with httpx.AsyncClient(default_encoding=autodecode,http2=True,headers=opt.UAheader,timeout=1,follow_redirects=True) as client:
        try:
            req_1 = await client.get(url='http://' + list_url)
            selector = parsel.Selector(req_1.text)
            selector_title = selector.xpath('/html/head/title').get().replace('<title>', "").replace('</title>',"").replace('\n',"")
            httpx_req_list='%s|%s|\033[33m%s\033[0m'%(req_1.status_code,selector_title,list_url.replace('\n',''))
            ...
            try:
                ...
                print('\033[32;40m',"||站点报错||",list_url.replace('\n',''),'\033[0m', end='\n')
######################################
